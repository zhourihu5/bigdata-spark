...
/**
 * 广告区域统计：ads_channel_analysis，区域维度：省份和城市
 */
object AdsChannelAnalysisReport {
	
	/**
	 * 基于DSL（调用Dataset API函数）分析数据
	 */
	def doReport(dataframe: DataFrame): DataFrame = {
	...
	def saveReportToMySQL(reportDF: DataFrame): Unit = {
		reportDF.write
			...
			.option("dbtable", "itcast_ads_report.ads_channel_analysis")
			.save()
	}
